...
\begin{document}
    ...
    \chapter{Introduction}\label{introduction}
        ...
        \section{EEG acquisition devices}
            ...
            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                \centering
                \includegraphics[width=0.8\textwidth]{placeholder.jpg} 
                \caption{Three different EEQ acquistion devices \todo{/ create picture with three priced devices}}\label{eeg-devices}
            \end{figure}
            ...
            \todo{\textbf{Amend and revisit this section after access to \cite{Zerafa.2018}}}
    ...
    \chapter{Survey Structure and layout}
        ...
        \section{Information Transfer Rate - ITR}\label{itr}

            Although \cite{Martins.2019} makes claims about the amount of data gathered from a potential fully connected brain down to the single neuron, this is only an educated guess solely based on the estimated number of neurons in every individual brain. From a research perspective this number has no real application yet. But there are several papers, which touched on the problem of a metric to measure the rate of data transmission from a human brain to a machine. This metric is crucial for the study in this thesis as there has to be definitive parameter to gauge the performance of any given participant in the experiment. \cite{Yuan.2013} stated that... Synchroneous, Asynchroneous, non-stable (memory), error correction, prediction, goal directed behavior

            \begin{itemize}
                \item no consensus about the metric
                \item cite different papers
                \item settle for one approach
            \end{itemize}            
