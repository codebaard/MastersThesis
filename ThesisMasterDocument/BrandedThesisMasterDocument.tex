...
\date{\sffamily Hamburg, 11.10.2021}  % Abgabedatum!
...
\begin{document}
    ...
    \chapter{Introduction}\label{introduction}
        ...
        \section{Working principle}\label{working-principle}         
            ...
            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                ...
                \caption{How SSVEP works in principle}\label{vep-principle}
            \end{figure}            
            ...
            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                \centering
                \includegraphics[width=1\textwidth]{IMG_9638.jpg} 
    ...
    \chapter{Study Framework}\label{survey-framework}
        ...
        \section{Statistical significance}\label{doe}
            ...
            Plots of data can be misleading in terms of accuracy or results regardless of how well the data was preprocessed. For this reason, it is necessary to show that the collected data actually exhibits a statistical significance in regards to the corresponding hypothesis. This section operationalizes the hypothesis in a way which makes it evaluable based on facts.

            \medskip

            The hypothesis states that \textit{"Age does not have a detrimental effect on the ability to use a non-invasive BCI based on VEPtechnology"} - see section \ref{hypothesis}. The question is how to measure the presence or absence of an effect. The usual method in all kinds of studies of qualitative or quantitative type is to disprove the so called \textit{null hypothesis} - $H_{0}$. The part of the hypothesis about \textit{"[...] non-invasive BCI on VEP technology[...]"} is to set the scope of the study in way that explicitly excludes other BCI technologies. The relevant part for the $H_{0}$ is \textit{"Age does not have a detrimental effect on the ability [...]"}, which poses the question how to measure abstract concepts such as \textit{detrimental} and \textit{ability}? One way to evaluate this is to conduct an experiment and ask the participants, and gain subjetive opinions about their individual performance. However, this will be inaccurate at best and totally ambiguous at worst. Apart from the fact the the study needs to conduct lots of trials in order to gain meaningful and coherent data, which is in contrast to the explorative character of this study, according to section \ref{study-scope}.

            A better way to do this is to define an objectively mesaurable metric and define threshold values beforehand to definitively reject the hypothesis. Section \ref{general-considerations} goes into detail regarding the capabilities of the sensor, thus we already know that the only available metric to choose from is the timing for neurotag activation. Using that information, we can already state that the null hypothesis will use timing as the fundamental metric.

            Nevertheless, before we can define a null hypothesis, we need to consider two other factors, both corresponding to error. There is the distinct risk that we do not reject the hypothesis although it is in fact wrong, which is called \textit{Type I Error} or \textit{false positive} and on the other side that the hypothesis is rejected although it is correct, which corresponds to a \textit{Type II Error} or \textit{false negative}. 

            These risks are depicted as $\alpha$ for the \textit{Type I Error} and $\beta$ for the \textit{Type II Error}:

            \paragraph{Significance Level Alpha - $\alpha$:} Alpha is the governing parameter, which decides how big the risk in a given sample size is that the hypothesis is wrong and therefore falsified. Since the hypothesis can be either true or false, the probability that it is wrong decreases with the number of samples taken in a binomial fashion \cite[103]{Siebertz.2017}. That means in this case that the likelihood $p$ of a participant to be above or below the threshold, which will be discussed in section \ref*{mafs}, is not determined by age. According to \cite[110]{Siebertz.2017} common values for $\alpha$ tend to be chosen in a way that the probability of a falsified hypothesis is at 1\%, 5\% and 10\%. Where a smaller value means a more strict threshold. To account for the explorative character of this study the value of 5\% is chosen. Therefore the susceptibility for unaccounted side-effects is reduced and the total number of participants is still not unfeasibly large. There is no distinct way to define that number, it is often chosen based on experience or strict requirements.
            ...
            In order to define these thresholds, we need to operationalize the abstract concepts of \textit{detrimental} and \textit{ability}. \textit{Detrimental} is fairly easy: in regards to timing it can only mean that the achieved timing is slower than that of a control group. \textit{Ability} on the other hand needs a definition in a frame of reference, which in this case also corresponds to the control group but in a different manner. This study collects numerous data points for each experiment in order to have a consistent dataset - see section \ref{experimental-protocol}. The established way of analyzing datasets of different numerical data points is by using statistics to calculate significance. By doing so, we can operationalize the null hypothesis in terms of standard deviation and means in regards to timings. In case the data shows that the null hypothesis is rejected, we need an alternative hypothesis, which states that there in fact is a influence of age. This alternative hypothesis - $H_{a}$ defines the boundaries for the possible solutions such that we can calculate $\beta$ later on.
            ...
            Having established the transformation of the dataset into a standard normal distribution, the $H_{0}$ can now be defined as a mathematical expression, where the control group is depicted as $CG$ and the group under evaluation as effect group or $EG$ in short. The \textit{Z-Score} depicts the corresponding mean timing of that particular age group:
            ...
            \begin{equation}\label{ha}
                Z_{EG} - Z_{CG} \geq 2\sigma
            \end{equation}            

            This translates to: The null hypothesis has to be rejected and the alternative hypothesis accepted, when the difference in timing between the younger and the older half of the participants is larger than two standard deviations.

            Under the assumption that in fact $Z_{EG}$ is larger than 2, the probability for the \textit{Type II Error} or $\beta$ can be defined using a fictional $\mu^{*}$, which will be determined in the experiment - see section \ref{mafs}:
        ...
        \section{Time vs Age correlation}\label{correlation}
            ...
            By collecting several timing data points for each participant, a regression graph between the age of the participants and the mean timinig for each visual cue can be drawn in order to show if there is a correlation of the parameters. Also these data points will be aggregated into a mean timing for each participant to have one definitive sample for each participant which removes potential outliers which might have been caused by external effects i.e. distraction.
    ...
    \chapter{Survey Contents}\label{survey-contents}
        ...
        \section{Remote control interface}
            ...
            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                ...
                \caption{Interface of the remote control experiment. Cues are highlighted}\label{gui-remote}
            \end{figure} 
        ...
        \section{Experimental Protocol}\label{experimental-protocol}
            ...
            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                \centering
                \includegraphics[width=1\textwidth]{IMG_9635.jpg} 
    ...
    \chapter{Survey results}\label{raw-results}
        ...
        \section{Raw results}
            ...
            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                ...
                \caption{Detection times filtered for t <= 10s}\label{time-hist-narrow}
            \end{figure} 
        ...
        \section{Demographic differences}\label{demographic-differences}
            ...
            In order to have a better understanding of the difference between younger and older participants, the whole population of the study has been divided into two different age groups:
            ...
            The results are show in figure \ref*{age-groups}. The most prominent feature is the age group above 40 years old. Their average time to detect a target is roughly between 5 and 10 seconds. Therefore the first preliminary finding purely based on visual evidence is that there in fact is a tendency that with increasing age the detection of a target takes longer.

            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                \centering
                \includegraphics[width=1\textwidth]{timeDistribution.png} 
                \caption{Detection times for two age groups}\label{age-groups}
            \end{figure}
    ...
    \chapter{Findings}\label{findings}
        ...
        \section{Mathematical proof}\label{mafs}
            ...
            Figure \ref*{regression} already showed that within a 95\% confidence interval given enough data points, the same study should also yield results within the same range if the parameters of the study are not altered. This section focusses on proving that the null hypothesis holds true with the method discussed in section \ref{doe}. Figure \ref{means-density} already exhibits that one group of participants is significantly outside of the main cluster of mean timings. From figure \ref{regression} it can also be derived that this group in fact represents the \textit{above 40 group}. The orange line printed on top is a KDF\footnote{Kernel Density Function} to highlight the influence of this group on a normalized sample.
            ...
            In order to transform the results of collected data in the dataset to a standard normal distribution, the sample mean has to be calculated in order to later transform all parameters into a standard normal distribution. Later on, the sample means of two distinct groups will be compared to prove or reject the null hypothesis. The groups will be separated as follows:
            ...
            where $\bar{X}$ corresponds to the mean of any single participant's timing and $n$ corresponds to the number of participants in the experiment. To calculate the standard deviation of the sample, which is required to determine the \textit{Z-Score} for any particular group, the following equation is utilized:
        ...
        \section{Other findings}\label{other-findings}
            ...
            The collected dataset allowed for other interesting insights into possible performance parameters of the participants, which were already briefly shown in the \ref*{raw-results}. This section briefly discusses these results and interpretes them in the context of this study.
    ...
    \chapter{Conclusion}
        ...
        \section{Results}
            ...
            To conclude this study, its reasonable to make a recapitulation of the goals of this study in order to evaluate whether they were met entirely:

            Section \ref{use case-main} established general considerations of a study in regards to BCI usage, deducted the core points, evaluated the constraints, and created a use case corresponding to these considerations and constraints. This concluded in a formulated hypothesis which stated that age does not have an influence on the ability to use a BCI. In contrast to that hypothesis sections \ref{findings-age} and \ref{mafs} proved that in fact the hypothesis is falsified. Which in turn means that age in fact \textit{does} have an effect on the ability to use a BCI. Especially section \ref{findings-age} tried to deliver a reasoning for that, which is presumed to be related to the ability to concentrate on a given task for prolonged periods of time. This effect is clearly visible in the age group of 40 years onwards, according to figure \ref{regression} and the box plot at figure \ref{age-dist}. Another indication is that the only two participants, who weren't able to deliver a good calibration score - resulting in not being able to undergo the experiment - were also in this age group. Another interesting factor, which shows that this study had a good design, is that almost none of the participants from very young age to the oldest ones reported difficulties in the task - see figure \ref*{survey-boxplots} : Difficulty. This makes the collected dataset even more valid, because this means that there was no bias through technical difficulties.
    ...
    \chapter{Acknowledgements}
        ...
        A special thanks goes to B.Sc. Moritz Bednorz, who helped me sharpen and clarify my knowledge in the field of medical engineering and design of experiment, which gave this thesis a coherent perspective on all domains and B.Sc. Niclas Lamponen who proof read the first manuscript.
        My biggest thanks are dedicated to my girlfriend B.A. Alexandra Michels for her emotional support and creative input on some of the figures in this thesis.
        I will be eternally grateful to my father Dr. Thomas Neudecker to set the right impulses in my life which made me pursue an academic degree. He may rest in peace. 
