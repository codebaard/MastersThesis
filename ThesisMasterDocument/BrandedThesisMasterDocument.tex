...
\begin{document}
    ...
    \chapter{Survey Framework}\label{survey-framework}
        ...
        \section{Statistical significance}\label{doe}
            ...
            Summarizing this section, a number of \todo{XXX} participants have to be tested, to say with a \todo{XX}\% certainty, whether the hypothesis holds true. 
    ...
    \chapter{Survey Contents}\label{survey-contents}
        ...
        \section{Age of the participants in the study}

            The objective of the study is to figure out whether age has an impact on the ability to use a BCI. Therefore it is important to have a reasonably big cross-section in regards in to age in the whole population of the sample. Figure \ref*{age-dist} shows the distribution of the age of all participants in the study. 
            ...
            This shows that the age of the participants was fairly evenly distributed across all age groups.
    ...
    \chapter{Findings}\label{findings}
        ...
        \section{Age as a factor in BCI usage}\label{findings-age}
            ...
            Section \ref*{correlation} discussed, how a regression curve will be used in order to identify a correlation between age and expected performance in regards to BCI usage. Figure \ref*{mean-time} shows this regression curve:

            \begin{figure}[h]     % h=here, t=top, b=bottom, p=page
                \centering
                \includegraphics[width=0.8\textwidth]{DetectionRegression.png} 
                \caption{Regression curve based on mean detection time per participant \todo{Add confidence interval}}\label{mean-time}
            \end{figure}
    ...
    \chapter{Conclusion}
        ...
        \section{Results}
            ...
            Age does have an influence. \todo{Elaborate a little more on that later.}

        \section{Future Work}

            There are several ways to continue with this resarch in subsequent studies, based on the outcome of this study. Some of them are briefly outlined in the following paragraphs:

            \paragraph{Gender} Based on the gender gap in section \ref*{other-findings}, there might be a factor in the difference in genders, which was not being discovered yet. Since there was no apparent gap in terms of average age, which might be an explanation, the female participants were in fact older. However, the results were inconclusive. Therefore a quantitative study to reveal a systematic influence might be worthwile.
